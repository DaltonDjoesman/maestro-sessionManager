...
The README SHALL describe the problem Maestro solves, state that it is a **session launcher** (not a window manager or session teardown tool), list the stack (Tauri 2, React, Rust, OpenSpec), provide quick-start build instructions, link to key docs, embed or link screenshots of the main surfaces (hub, capture, activation results when available), and include a short motion demo (GIF or animated WebP) of the core hub→activate loop (capture optional). The README docs map (or equivalent links section) SHALL include `docs/follow-up-craft.md` when that file exists. The README SHALL state that the default UI locale is pt-PT and that any English locale file is a scaffold only until real translations and a user-facing locale switch exist.
...
#### Scenario: Demo motion media present

- **WHEN** a visitor reads the README
- **THEN** they SHALL find an embedded or linked short GIF/WebP demonstrating activating a session from the hub (or hub→capture→activate)
