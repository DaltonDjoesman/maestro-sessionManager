...
impl ApplicationSettings {
    ...
    /// Identity normalize; legacy keys (e.g. `logging_verbosity`,
    /// `assisted_profile_capture_enabled`, `default_browser_executable`,
    /// `default_browser_family`) are ignored on deserialize and omitted on serialize.
...
mod tests {
    ...
    fn serializes_with_snake_case_fields() {
        ...
        assert!(!json.contains("logging_verbosity"));
    ...
    #[test]
    fn ignores_legacy_logging_verbosity_and_browser_defaults_on_deserialize() {
        let raw = r#"{
            "schema_version": 1,
            "profiles_root": "/tmp/p",
